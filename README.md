...
| `api/` | 完成：`POST /api/applicants`、`POST /api/applicants/{id}/verify`、`sms/send`、`sms/verify`、`account-setup`、`reset`（含真的接 PostgreSQL）。`sms/verify` 產生的 `sessionId` 之後要放在 `X-Session-Id` header 帶給其他端點，細節見 `PHASE1_NOTES.md` §七第 7 點。CORS 已開放（開發階段 `allow_origins=["*"]`，正式環境要改成前端實際網域）。`admin/login`、`admin/records`、`id-card/rectify` 與 `frontend/` 尚未開始 |
