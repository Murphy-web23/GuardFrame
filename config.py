...
# 2026-09-09：原本這裡跟 analyzer.py 一起用 abs(latency_ms) 判定，
# 不分正負——上面這段舊註解提到的「真人測到 -468.6ms」是這樣做的
# 理由，但那次測試（2026-08-25）比修好「照明序列容易連續抽到同一
# 顏色」那個 bug（見 frontend/src/utils/verificationRecording.ts
# generateLightLog()，2026-09-07 修的）早了兩週。顏色重複會讓螢幕
# 亮度曲線接近週期性重複，正是互相關演算法在雜訊裡「對到錯誤週期」、
# 算出負延遲假峰值的典型成因——那次的 -468.6ms 很可能是撞到那個舊
# bug，不是管線延遲真的會是負的。_cross_correlate() 自己的文件定義
# 「正值代表反射晚於光源變化」，負延遲在物理上就是反射發生在刺激
# 之前，是真的不合理。真人測試（applicant 1770，虛擬攝影機注入攻擊）
# 量到 -400.35ms 卻被 abs() 判定為合理放行，證實這個漏洞。改成不對稱
# 的容忍範圍：允許小幅度負延遲（量測雜訊），大幅負延遲一律不合理。
PHOTO_LATENCY_MIN_MS = -50
